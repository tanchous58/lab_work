...
df = pd.read_csv('концепт.csv', sep=';',encoding='cp1251',header=0, index_col=0)
df = df.iloc[np.random.permutation(len(df))]

X = df.iloc[0:90, 0:6].values
y = df.iloc[0:90, 6]
y = y.map({'продвинутый': 1, 'достаточный': 2, 'удовл':3}).values.reshape(-1,1)
Y = np.zeros((y.shape[0], np.unique(y).shape[0]))
for i in np.unique(y):
    Y[:,i-1] = np.where(y == i, 1, 0).reshape(1,-1)

X_test = df.iloc[91:100, 0:6].values
y = df.iloc[91:100, 6]
y = y.map({'продвинутый': 1, 'достаточный': 2, 'удовл':3}).values.reshape(-1,1)
